...
public class WXServer1 {
	...
	public String findvhicinfo(String vhic) {
		String sql = null;
		String wxxx = "select a.*,rm.RM_MALFUNCTION RM_MALFUNCTION1,rt.rt_type,ra.ra_addr,us.real_name shry,uw.real_name shry_name"
				+ " ,v.mdt_no,v.sim_num,v.mt_name,v.owner_name,v.comp_name,v.mdt_sub_type from(select r.*"
				...
				+ " left join VW_VEHICLE_WX v on a.vehi_no = v.vehi_no "
				+ " order by rr_time desc";
		List<Map<String, Object>> list =null;
		sql = "select * from VW_VEHI_MDT_WX where vehi_no = '"+vhic+"'";
	...
	public String vhicinfo(String vhic) {
		...
		for (int i = 0; i < l.size(); i++) {
			l.get(i).put("SXSJ", l.get(i).get("RR_TIME")==null?"":String.valueOf(l.get(i).get("RR_TIME")));
			l.get(i).put("WXSJ", l.get(i).get("RR_TIME_END")==null?"":String.valueOf(l.get(i).get("RR_TIME_END")));
			...
			l.get(i).put("IS_REPAIRED",l.get(i).get("IS_REPAIRED")==null?"":(l.get(i).get("IS_REPAIRED").toString().equals("0")?"修复":(l.get(i).get("IS_REPAIRED").toString().equals("1")?"未修复":"")));
			l.get(i).put("IS_TRANSFER_ORDER",l.get(i).get("IS_TRANSFER_ORDER")==null?"":(l.get(i).get("IS_TRANSFER_ORDER").toString().equals("0")?"未转单":(l.get(i).get("IS_TRANSFER_ORDER").toString().equals("1")?"已转单":"")));
			l.get(i).put("TRANSFER_ORDER_STATUS",l.get(i).get("TRANSFER_ORDER_STATUS")==null?"":(l.get(i).get("TRANSFER_ORDER_STATUS").toString().equals("0")?"待确认":(l.get(i).get("TRANSFER_ORDER_STATUS").toString().equals("1")?"已确认":"")));
	...
	public String wxcc(String postData){
		...
		String WXLX = String.valueOf(paramMap.get("WXLX"));//维修方式
		...
		String sql = "select * from(select vehi_no,count(1) c from TB_REPAIR_RECORD t,TB_USER_WX u,TB_REPAIR_TYPE r where"
				...
				if(WXLX!=null&&!WXLX.equals("null")&&WXLX.length()>0&&!WXLX.equals("所有维修方式")){
					sql += " and rt_type = '"+WXLX+"'";
				}
								
				sql += " group by vehi_no)a left join VW_VEHICLE_WX v on a.vehi_no = v.vehi_no where 1=1";
...
//		List<Map<String, Object>> l = jdbcTemplate.queryForList(wxxx);
		...
		sql+= ") al left join TB_REPAIR_MALFUNCTION trm on al.rm_id = trm.rm_id"
				...
				+ " left join TB_USER_WX us on al.RR_ASSESSOR=us.user_id left join VW_VEHICLE_WX v"
	...
	public String addwxjl(String postData,HttpServletRequest request){
		...
		String rt_id = String.valueOf(paramMap.get("rt_id")).equals("null")?"":String.valueOf(paramMap.get("rt_id"));//维修方式id
		...
		String rr_time = String.valueOf(paramMap.get("rr_time")).equals("null")?"":String.valueOf(paramMap.get("rr_time"));//报修日期
		String rr_time_end = String.valueOf(paramMap.get("rr_time_end")).equals("null")?"":String.valueOf(paramMap.get("rr_time_end"));//维修日期
		String rm_id = String.valueOf(paramMap.get("rm_id")).equals("null")?"":String.valueOf(paramMap.get("rm_id"));//故障类型
		String rm_malfunction = String.valueOf(paramMap.get("rm_malfunction")).equals("null")?"":String.valueOf(paramMap.get("rm_malfunction"));//故障类型
	...
	public String finddhhf(String postData){
		...
		sql+= ") al left join TB_REPAIR_MALFUNCTION trm on al.rm_id = trm.rm_id"
				...
				+ " left join VW_VEHICLE_WX v on al.vehi_no=v.vehi_no where 1=1";
...
//				+ " from TB_REPAIR_RECORD t,tb_user u,TB_REPAIR_TYPE rt,VW_VEHICLE_WX v,TB_REPAIR_ADDR a,TB_REPAIR_PHONE rth"
