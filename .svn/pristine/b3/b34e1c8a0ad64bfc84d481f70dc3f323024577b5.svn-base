...
public class CommonServer {    
	...
	public String findzdzlx(){
		String sql = "select sub_name,subid from TB_MDT_SUB_TYPE t";
		List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
		List al = new ArrayList();
		Map map1 = new HashMap();
		map1.put("name", "所有子类型");
		...
		for(int i=0;i<list.size();i++){
			...
			map.put("id", list.get(i).get("sub_name"));
			al.add(map);
		}
		Map m = new HashMap();
		m.put("datas", al);
	...
	public String findwxlx(){
		String sql = "select * from TB_REPAIR_TYPE order by rt_type";
		List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
		List al = new ArrayList();
		Map map1 = new HashMap();
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			try {
				...
				for (FileItem item : items) {
					...
					if(name.equals("RR_ID")){
						rr_id =  br.readLine();
                    }
				}
				if(rr_id!=null&&!String.valueOf(rr_id).equals("null")&&!String.valueOf(rr_id).equals("")){
					jdbcTemplate.update("update TB_REPAIR_RECORD set RR_PICT='',RR_END_PICT='',RR_WXD_PICT='' where rr_id = '"+rr_id+"'");
					request.getSession().setAttribute("RR_ID", rr_id);
				}else{
					rr_id = findMaxId()+"";
					jdbcTemplate.update("insert into TB_REPAIR_RECORD (RR_ID) values ('"+rr_id+"')");
					request.getSession().setAttribute("RR_ID", rr_id);
				}
				for (FileItem item : items) {
					String name = item.getFieldName();
						...
						if (!item.isFormField()) {
							...
							try {
								...
								if (item.getFieldName().equals("RR_PICT1")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
										if(String.valueOf(list.get(0).get("RR_PICT")).equals("null")){
											a = jdbcTemplate.update(
													"update TB_REPAIR_RECORD set RR_PICT ='"+in+"' where RR_ID = ?",
													new Object[] {rr_id});
											if (a == 0) {
											...
											a = jdbcTemplate.update(
													"update TB_REPAIR_RECORD set RR_PICT =RR_PICT|| ';"+in+"' where RR_ID = ?",
													new Object[] {rr_id});
											if (a == 0) {
												return "{\"code\":400,\"data\":\"图片提交失败\"}";
											} else {
...
//										tp = 1;
//									}
								} else if (item.getFieldName().equals("RR_END_PICT1")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
										if(String.valueOf(list.get(0).get("RR_END_PICT")).equals("null")){
											a = jdbcTemplate.update(
													"update TB_REPAIR_RECORD set RR_END_PICT ='"+in+"' where RR_ID = ?",
													new Object[] {rr_id});
											if (a == 0) {
												return "{\"code\":400,\"data\":\"图片提交失败\"}";
											} else {
											...
											a = jdbcTemplate.update(
													"update TB_REPAIR_RECORD set RR_END_PICT =RR_END_PICT|| ';"+in+"' where RR_ID = ?",
													new Object[] {rr_id});
											if (a == 0) {
												return "{\"code\":400,\"data\":\"图片提交失败\"}";
											} else {
...
//										tp = 1;
//									}
								} else if (item.getFieldName().equals("RR_END_PICT2")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
...
//										tp = 1;
//									}
								} else if (item.getFieldName().equals("RR_END_PICT3")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
...
//										tp = 1;
//									}
								} else if (item.getFieldName().equals("RR_WXD_PICT1")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
										if(String.valueOf(list.get(0).get("RR_WXD_PICT")).equals("null")){
											a = jdbcTemplate.update(
													"update TB_REPAIR_RECORD set RR_WXD_PICT ='"+in+"' where RR_ID = ?",
													new Object[] {rr_id});
											if (a == 0) {
												return "{\"code\":400,\"data\":\"图片提交失败\"}";
											} else {
											...
											a = jdbcTemplate.update(
													"update TB_REPAIR_RECORD set RR_WXD_PICT =RR_WXD_PICT|| ';"+in+"' where RR_ID = ?",
													new Object[] {rr_id});
											if (a == 0) {
												return "{\"code\":400,\"data\":\"图片提交失败\"}";
											} else {
...
//										tp = 1;
//									}
								} else if (item.getFieldName().equals("RR_WXD_PICT2")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
...
//										tp = 1;
//									}
								} else if (item.getFieldName().equals("RR_WXD_PICT3")) {
									int a = 0;
									String sql = "select * from TB_REPAIR_RECORD where rr_id = ?";
									List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, rr_id);
									if(list.size()>0){
...
//									}
	...
	public void query_pic(HttpServletRequest request, HttpServletResponse response,String type, String rr_id, int info) {
		try {
			String sql = "select RR_PICT,RR_END_PICT,RR_WXD_PICT from TB_REPAIR_RECORD where rr_id = '"+ rr_id+"'";
			List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
			String url = "";
			if(type.equals("0")){
				url = list.get(0).get("RR_PICT").toString().split(";")[info];
			}else if(type.equals("1")){
				url = list.get(0).get("RR_END_PICT").toString().split(";")[info];
			}else if(type.equals("2")){
				url = list.get(0).get("RR_WXD_PICT").toString().split(";")[info];
			}
...
//				}
	...
	public String findParts() {
		String sql = "select PARTS_ID, PARTS_NAME, PARTS_PRICE from TB_REPAIR_PARTS";
		List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
