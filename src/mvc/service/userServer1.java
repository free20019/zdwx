...
public class userServer1 {
	...
	public String finduser(String info){
		...
		for (int i = 0; i < list.size(); i++) {
			list.get(i).put("APP_POWER_NAME",list.get(i).get("APP_POWER")==null?"":
					(list.get(i).get("APP_POWER")).toString().equals("0")?"全部维修记录":(list.get(i).get("APP_POWER")).toString().equals("1")?"当前用户维修记录":"");
		}
	...
	public String adduser(String postData){
		...
		String app_power = String.valueOf(paramMap.get("app_power"));
		String phone_number = String.valueOf(paramMap.get("phone_number"));
		int user_id = findMaxId("TB_USER_WX","user_id");
		int count = 0;
		String sql = "insert into TB_USER_WX (user_name,user_pwd,real_name,note,ba_id,comp_id,app_power,phone_number,user_id) values "
			+ " ('"+user_name+"','"+user_pwd+"','"+real_name+"','"+note+"','11037','11037001','"+app_power+"','"+phone_number+"','"+user_id+"')";
	...
	public String edituser(String postData){
		...
		String sql = "update TB_USER_WX set user_name='"+user_name+"',user_pwd='"+user_pwd+
				"',real_name='"+real_name+"',note='"+note+"',app_power='"+app_power+"',phone_number='"+phone_number+"' where user_id='"+user_id+"'";
		count = jdbcTemplate2.update(sql);
		Map<String, Object> map = new HashMap<String, Object>();
		if(count>0){
			map.put("info", "修改成功");
